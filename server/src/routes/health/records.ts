...
import { assessQuality } from "../../../shared/health-quality.js";
import {
  RECORD_FIELDS,
  RECORD_SELECT,
  RECORD_ORDER,
...
/** GET /api/health/records —— 分页 + 可选日期范围 / 来源，按 record_date 倒序 */
router.get("/health/records", authMiddleware, (req, res) => {
  ...
  const { whereSql, params, error } = recordFilter(userId, q);
  if (error) {
 ...
 *
 * C5：导出属关键操作 → 留痕（只记条数与筛选条件，不含任何指标数值）。
 */
router.get("/health/records/export", authMiddleware, (req, res) => {
  const q = req.query as Record<string, string | undefined>;
  const { whereSql, params, error } = recordFilter(req.user!.id, q);
  if (error) {
    res.status(400).json({ code: 40001, message: error });
    return;
  }
 ...
 * C2：来源固定为 `manual`（手动录入），客户端无法通过入参伪造来源；质量标记由共享层现算。
 */
...
router.post("/health/records/import", authMiddleware, adminOnly, (req, res) => {
  ...
  // C2：来源固定 import；身高在循环外查一次（BMI 质量推导用），避免逐行查库
  ...
  // 任一行不合法 → 整批不写（C5：失败的批次同样留痕，只记行数不记行内容）
  if (errors.length) {
    writeAudit(
      "records_import",
      { total: list.length, success: 0, fail: errors.length, rejected: true },
      actorOf(req)
    );
    res.json({
      code: 0,
 ...
 *
 * C2：指标被改动后按**库内合并值**重算 quality_flag（只带 remark 的更新不会触发重算），
 * 保证质量标记永远与库内数值一致 —— 例如把收缩压从 120 改成 999，标记会随之变为 invalid。
 * 字段映射改用共享的 `RECORD_FIELDS`（原先此处内联了一份副本，容易与新增/导入路径漂移）。
 */
router.put("/health/records/:id", authMiddleware, (req, res) => {
  ...
  if (sets.length) {
    const touchedMetric = Object.keys(changed).some(k => k !== "record_date");
    if (touchedMetric) {
      const merged = { ...rowColumnValues(current), ...changed };
      sets.push("quality_flag = @quality_flag");
      params.quality_flag = assessQuality(
        qualitySubjectOf(merged),
        profileHeight(userId)
      ).flag;
    }
    db.prepare(
