...
// C5：登录审计（成功 / 失败）统一走审计助手；助手放在 health 路由目录下（见 C5 方案指引），
// 它只依赖共享层与 db，import 进来不会挂载任何路由。
import { writeAudit } from "./health/audit.js";
 ...
 *
 * C5：登录成功与**每一种失败**都写审计。失败时账号可能根本不存在，
 * 因此 user_id 记 NULL、把**尝试的用户名**记进 actor_name（明细只记原因，不记密码）。
 */
router.post("/login", (req, res) => {
  const { username, password } = req.body ?? {};
  const attempted = typeof username === "string" ? username.trim() : "";
  const user = attempted ? findUserByName(attempted) : undefined;
  const actor = {
    userId: user?.id ?? null,
    username: attempted,
    ip: req.ip ?? ""
  ...
  if (Number(user.status) === 0) {
    writeAudit("login_failed", { reason: "账号已停用" }, actor);
    accountDisabled(res);
    return;
  }

  const roles = splitRoles(user.roles);
  writeAudit("login_success", { roles }, actor);
  res.json({
    code: 0,
    message: "操作成功",
    data: {
