...
/** C2：被过滤掉的无效记录条数（页面提示用，说明趋势图为何少了点） */
...
function chartOption() {
  ...
  return {
    ...
    xAxis: {
      type: "category",
      data: data.map(t => t.date.slice(5)),
...
async function loadData() {
  ...
  try {
    ...
    if (code === 0 && data) {
      // C2：趋势图与报告 / 评分同口径 —— 质量标记为 invalid 的记录不参与展示，
      // 过滤用的是共享层唯一的判定函数（与服务端 recordsForEngine 同一个）。
      const all = data.list ?? [];
      records.value = all.filter(isAnalyzable);
      invalidCount.value = all.length - records.value.length;
    }
    await nextTick();
    renderChart();
  } finally {
...
<template>
  <div v-loading="loading">
    <el-card shadow="never">
      ...
      <!-- C2：无效数据提示（与报告 / 评分同口径，说明趋势图为何少了记录） -->
      <el-alert
        v-if="invalidCount"
        type="info"
        show-icon
        :closable="false"
        class="mb-4"
        :title="`已排除 ${invalidCount} 条质量标记为「无效」的记录`"
        description="这些记录的数值超出可测量范围（如收缩压 999），不参与趋势、报告与评分；可在历史记录页按质量标签查看。"
      />
